...
int main()
{
    //other variables
    std::thread t(a_func);
    thread_guard g(t); //对象 g 被第一个销毁->join()被调用,即使下面函数抛出异常
...
1. C++中通过实例化 `std::mutex` 创建互斥量实例，通过`mutex`成员函数`lock()`对互斥量上锁，`unlock()`进行解锁
...
1. 避免死锁的一般建议
   1. 避免嵌套锁: 一个线程已经获得一个锁时，不去获得第二个
   2. 避免在持有锁时调用用户提供的代码：无法确保用户代码是否要求锁，可能违反上一条规则
   3. 使用固定顺序获取锁：当要求两个或两个以上的锁，且不能使用`std::lock`同时锁住，那么每个线程都应用相同固定的加锁顺序
   4. 使用锁的层次结构：当要对一个互斥量上锁，在该层锁已被低层持有时，上锁是不允许的，保证了枷锁顺序
